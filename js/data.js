const CASES = [
  {
  "id": "na-uspenskoy",
  "title": "ЖК На Успенской",
  "address": "Уфа, ул. Коммунистическая, 107",
  "areaSqm": null,
  "areaLabel": "площадь уточняется",
  "roomsLabel": "3-комнатная квартира",
  "format": "Дизайн + ремонт под ключ",
  "style": "Современная классика",
  "segment": "Комфорт",
  "complex": "ЖК На Успенской",
  "tags": [
    "3-комнатная",
    "кухня",
    "детская",
    "видео"
  ],
  "rooms": [
    "кухня",
    "гостиная",
    "спальня",
    "детская",
    "санузел"
  ],
  "hasProjectRender": false,
  "hasVideo": true,
  "cover": "assets/na-uspenskoy/after-1.png",
  "summary": "Трёхкомнатная квартира в ЖК На Успенской: дизайн-проект и ремонт под ключ в стиле современной классики — молдинги, золотой свет, кухня с фиолетовым акцентом и детская с картой мира.",
  "task": "Заказчик хотел светлую семейную квартиру с ощущением «дорогого, но не перегруженного» интерьера: тёплые бежевые стены, лепнина и многоуровневый свет в гостиной, выразительная кухня, уютная спальня с фотообоями и отдельная детская с характером. Важно было заранее согласовать отделку, встроенную мебель и освещение по проекту и получить ремонт под ключ без сюрпризов на финале.",
  "objectDescription": "Выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. В гостиной и коридоре — декоративные молдинги, люстры-кольца и бра; на кухне — двухцветные фасады, мраморный фартук и кольцевые светильники; в спальне — рифлёные панели, фотообои и хрустальная люстра; в детской — синие стены и фотопанно с картой мира; в санузле — мраморная плитка, встроенная прачечная и шкафы до потолка. Квартира передана в состоянии, готовом к проживанию.",
  "workList": [
    "Демонтаж старых покрытий и подготовка оснований",
    "Полная замена электрики и сантехники",
    "Штукатурка и выравнивание стен, подготовка под финишную отделку",
    "Устройство стяжки и выравнивание пола",
    "Монтаж декоративных молдингов, карнизов и рифлёных панелей",
    "Чистовая отделка: плитка, ламинат, покраска и фотообои",
    "Монтаж натяжных потолков, люстр, бра и точечного света",
    "Изготовление и монтаж встроенной мебели (кухня, шкафы в санузле)",
    "Установка межкомнатных дверей и плинтусов",
    "Монтаж кухонного фартука, сантехники и бытовой техники",
    "Устройство ниши под стиральную и сушильную машины",
    "Установка розеток, выключателей, кондиционеров и финальная уборка"
  ],
  "highlights": [
    "Современная классика с молдингами и золотым светом",
    "Кухня с фиолетовым акцентом и мраморным фартуком",
    "Спальня с фотообоями и рифлёными панелями",
    "Детская с картой мира и санузел с прачечной",
    "Квартира передана готовой к проживанию"
  ],
  "clientFit": [
    "Семья с детьми — нужны и «взрослые» зоны, и детская с характером",
    "Хотите классику без перегруза: молдинги, свет и встроенная мебель в одном проекте",
    "Важна кухня, которая выглядит как в дизайн-проекте, а не «потом докупим»",
    "Сравниваете подрядчиков по реальным объектам в новостройках Уфы"
  ],
  "gallery": [
    {
      "src": "assets/na-uspenskoy/after-1.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/na-uspenskoy/after-2.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/na-uspenskoy/after-3.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/na-uspenskoy/after-4.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/na-uspenskoy/after-5.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/na-uspenskoy/after-6.png",
      "alt": "После ремонта",
      "caption": ""
    }
  ],
  "video": {
    "label": "Видео с комментариями эксперта",
    "embedUrl": null,
    "externalUrl": null,
    "fileUrl": "assets/na-uspenskoy/expert-overview.mp4",
    "poster": "assets/na-uspenskoy/after-1.png",
    "note": ""
  },
  "cta": {
    "phone": "+7 (917) 766-09-38",
    "whatsapp": "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
    "telegram": "https://t.me/+79177660938",
    "max": "https://max.ru/chat?phone=%2B79641995283",
    "vk": "https://vk.com/za.komfortom",
    "quiz": "https://zakomfortom.com/quiz",
    "buttonLabel": "Обсудить похожий ремонт"
  }
},
  {
  "id": "kondi-nova",
  "title": "ЖК Конди Нова",
  "address": "Уфа, ул. Менделеева, 132/1",
  "areaSqm": 69,
  "areaLabel": "69 м²",
  "roomsLabel": "3-комнатная квартира",
  "format": "Дизайн + ремонт под ключ",
  "style": "Современный лофт",
  "segment": "Комфорт",
  "complex": "ЖК Конди Нова",
  "tags": [
    "69 м²",
    "3-комнатная",
    "лофт",
    "видео"
  ],
  "rooms": [
    "гостиная",
    "кухня",
    "спальня",
    "санузел"
  ],
  "hasProjectRender": false,
  "hasVideo": true,
  "cover": "assets/kondi-nova/after-1.png",
  "summary": "Трёхкомнатная квартира 69 м² в ЖК Конди Нова: дизайн-проект и ремонт под ключ в стиле современного лофта — декоративный кирпич, скрытые двери, встроенная мебель и продуманное освещение.",
  "task": "Заказчик хотел интерьер с характером: не холодный минимализм, а сочетание кирпича и бетонных фактур с уютом и тёплым светом. Важны были скрытые двери, выразительная гостиная с акцентной стеной, рабочие места в спальнях, гардеробная и компактная прачечная в санузле — всё по проекту и под ключ, без сюрпризов на финале.",
  "objectDescription": "На площади 69 м² выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. В гостиной — кирпичная акцентная стена с LED-подсветкой, трековый свет и белая кухня; в коридоре — скрытые двери и рифлёные панели; в ТВ-зоне — декоративные обои и подвесная мебель. В спальнях — декоративный кирпич, подсветка изголовья, встроенные рабочие зоны и гардеробная со стеклянными фасадами. В санузле — каменная плитка, встроенная стиральная и сушильная машины, деревянная мебель у раковины. Квартира передана в состоянии, готовом к проживанию.",
  "workList": [
    "Демонтаж старых покрытий и подготовка оснований",
    "Полная замена электрики и сантехники",
    "Штукатурка и выравнивание стен, подготовка под финишную отделку",
    "Устройство стяжки и выравнивание пола",
    "Декоративный кирпич, штукатурка под бетон и рифлёные панели",
    "Чистовая отделка: плитка, ламинат, покраска и декоративные обои",
    "Монтаж трекового и точечного света, люстр и LED-контурной подсветки",
    "Изготовление и монтаж встроенной мебели (кухня, ТВ-зона, спальни, гардеробная)",
    "Установка скрытых межкомнатных дверей и плинтусов",
    "Монтаж кухонного фартука, сантехники и бытовой техники",
    "Устройство ниши под стиральную и сушильную машины",
    "Установка розеток, выключателей, кондиционеров и финальная уборка"
  ],
  "highlights": [
    "Лофт с кирпичом, бетонными фактурами и тёплым LED-светом",
    "Скрытые двери и рифлёные панели в коридоре",
    "Гардеробная со стеклянными фасадами и рабочие зоны в спальнях",
    "Санузел с прачечной и встроенной техникой",
    "Квартира передана готовой к проживанию"
  ],
  "clientFit": [
    "Нравится лофт, но не хочется «холодного бетона» — нужен уют и свет",
    "Важны скрытые двери, гардеробная и рабочие места — всё в одном проекте",
    "Ищете подрядчика, который делает и отделку, и встроенную мебель под ключ",
    "Смотрите на реальные объекты, прежде чем выбирать студию"
  ],
  "gallery": [
    {
      "src": "assets/kondi-nova/after-1.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-2.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-3.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-4.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-5.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-6.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-7.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-8.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-9.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-10.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-11.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-12.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kondi-nova/after-13.png",
      "alt": "После ремонта",
      "caption": ""
    }
  ],
  "video": {
    "label": "Видео с комментариями эксперта",
    "embedUrl": null,
    "externalUrl": null,
    "fileUrl": "assets/kondi-nova/expert-overview.mp4",
    "poster": "assets/kondi-nova/after-1.png",
    "note": ""
  },
  "cta": {
    "phone": "+7 (917) 766-09-38",
    "whatsapp": "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
    "telegram": "https://t.me/+79177660938",
    "max": "https://max.ru/chat?phone=%2B79641995283",
    "vk": "https://vk.com/za.komfortom",
    "quiz": "https://zakomfortom.com/quiz",
    "buttonLabel": "Обсудить похожий ремонт"
  }
},
  {
  "id": "kvartal-entuziastov-2",
  "title": "ЖК Квартал Энтузиастов 2",
  "address": "Уфа, ул. Рудольфа Нуреева, 1",
  "areaSqm": null,
  "areaLabel": "площадь уточняется",
  "roomsLabel": "квартира",
  "format": "Дизайн + ремонт под ключ",
  "style": "Современный интерьер с акцентами",
  "segment": "Комфорт",
  "complex": "ЖК Квартал Энтузиастов 2",
  "tags": [
    "семейная",
    "детские",
    "кухня",
    "видео"
  ],
  "rooms": [
    "гостиная",
    "кухня",
    "спальня",
    "детская",
    "санузел"
  ],
  "hasProjectRender": false,
  "hasVideo": true,
  "cover": "assets/kvartal-entuziastov-2/after-1.png",
  "summary": "Квартира в ЖК Квартал Энтузиастов 2: дизайн-проект и ремонт под ключ с выразительными цветовыми акцентами, встроенной мебелью, трековым светом и продуманными детскими зонами.",
  "task": "Заказчик хотел современную семейную квартиру с характером: не «белая коробка», а продуманные акценты — синий диван, тёмно-зелёная кухня, уютная спальня и функциональные детские. Важно было заранее согласовать отделку, встроенную мебель и освещение по проекту и получить ремонт под ключ без сюрпризов на финале.",
  "objectDescription": "Выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. В гостиной — синий акцент и рифлёная перегородка, в кухне — двухцветные фасады и мраморный фартук, в спальне — тёмно-зелёная стена и молдинги, в детских — зонирование и встроенные рабочие места, в санузле — плитка с подсветкой. Квартира передана в состоянии, готовом к проживанию.",
  "workList": [
    "Демонтаж старых покрытий и подготовка оснований",
    "Полная замена электрики и сантехники",
    "Штукатурка и выравнивание стен, подготовка под финишную отделку",
    "Устройство стяжки и выравнивание пола",
    "Декоративные перегородки, молдинги и рифлёные панели",
    "Чистовая отделка: плитка, ламинат/паркет, покраска стен",
    "Монтаж трекового и точечного света, люстр и LED-подсветки",
    "Изготовление и монтаж встроенной мебели (кухня, шкафы, ТВ-зона, детские)",
    "Установка межкомнатных дверей и плинтусов",
    "Монтаж кухонного фартука, сантехники и бытовой техники",
    "Установка розеток, выключателей и финальная уборка"
  ],
  "highlights": [
    "Синий и зелёный акценты в единой современной палитре",
    "Рифлёная перегородка и трековый свет в гостиной",
    "Детские с зонированием и встроенными рабочими местами",
    "Кухня с мраморным фартуком и двухцветными фасадами",
    "Квартира передана готовой к проживанию"
  ],
  "clientFit": [
    "Семья с детьми — нужны яркие акценты, но без хаоса в отделке",
    "Не хотите «белую коробку» — важны цвет, свет и встроенная мебель в одном проекте",
    "Нужны детские с рабочими зонами, а не просто «комната с кроватью»",
    "Уже сравниваете студии и хотите опереться на сданный объект"
  ],
  "gallery": [
    {
      "src": "assets/kvartal-entuziastov-2/after-1.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-2.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-3.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-4.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-5.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-6.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-7.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-8.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-9.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-10.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/kvartal-entuziastov-2/after-11.png",
      "alt": "После ремонта",
      "caption": ""
    }
  ],
  "video": {
    "label": "Видео с комментариями эксперта",
    "embedUrl": null,
    "externalUrl": null,
    "fileUrl": "assets/kvartal-entuziastov-2/expert-overview.mp4",
    "poster": "assets/kvartal-entuziastov-2/after-1.png",
    "note": ""
  },
  "cta": {
    "phone": "+7 (917) 766-09-38",
    "whatsapp": "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
    "telegram": "https://t.me/+79177660938",
    "max": "https://max.ru/chat?phone=%2B79641995283",
    "vk": "https://vk.com/za.komfortom",
    "quiz": "https://zakomfortom.com/quiz",
    "buttonLabel": "Обсудить похожий ремонт"
  }
},
  {
  "id": "novaland",
  "title": "ЖК Новаленд",
  "address": "Уфа, проспект Дружбы Народов",
  "areaSqm": null,
  "areaLabel": "площадь уточняется",
  "roomsLabel": "квартира",
  "format": "Дизайн + ремонт под ключ",
  "style": "Современный интерьер с природными акцентами",
  "segment": "Комфорт",
  "complex": "ЖК Новаленд",
  "tags": [
    "природные акценты",
    "кухня",
    "гостиная",
    "видео"
  ],
  "rooms": [
    "гостиная",
    "кухня",
    "санузел"
  ],
  "hasProjectRender": false,
  "hasVideo": true,
  "cover": "assets/novaland/after-1.png",
  "summary": "Квартира в ЖК Новаленд: дизайн-проект и ремонт под ключ с тёплыми деревянными текстурами, декоративным кирпичом, ёлочкой на полу и зонированием пространства.",
  "task": "Заказчик хотел светлую современную квартиру с характером: натуральные материалы, удобная кухня-столовая, уютная гостиная с зонированием и санузел без перегруза. Важно было заранее согласовать отделку, встроенную мебель и освещение по проекту и получить ремонт под ключ без сюрпризов на финале.",
  "objectDescription": "Выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. В гостиной — деревянная перегородка с ТВ-нишей и фотообои с лесным пейзажем, в кухне — двухцветные фасады и трековый свет, в санузле — тёмная плитка с LED-подсветкой. Квартира передана в состоянии, готовом к проживанию.",
  "workList": [
    "Демонтаж старых покрытий и подготовка оснований",
    "Полная замена электрики и сантехники",
    "Штукатурка и выравнивание стен, подготовка под финишную отделку",
    "Устройство стяжки и выравнивание пола",
    "Декоративный кирпич, фотообои и деревянные перегородки",
    "Чистовая отделка: плитка, ламинат/паркет «ёлочкой», покраска стен",
    "Монтаж трекового и точечного света, люстр и LED-подсветки",
    "Изготовление и монтаж встроенной мебели (кухня, шкафы, ТВ-зона)",
    "Установка межкомнатных дверей, балконных блоков и плинтусов",
    "Монтаж кухонного фартука, сантехники и бытовой техники",
    "Установка розеток, выключателей и финальная уборка"
  ],
  "highlights": [
    "Деревянные текстуры и декоративный кирпич в единой палитре",
    "Гостиная с перегородкой, ТВ-нишей и фотообоями",
    "Паркет «ёлочкой» и трековый свет на кухне",
    "Санузел с тёмной плиткой и LED-подсветкой",
    "Квартира передана готовой к проживанию"
  ],
  "clientFit": [
    "Хотите натуральные материалы и зонирование — без «офисного» минимализма",
    "Важны дерево, кирпич и свет в одном проекте, а не набор разных подрядчиков",
    "Нужна гостиная с характером и санузел без визуального шума",
    "Смотрите на реальные объекты в новостройках Уфы"
  ],
  "gallery": [
    {
      "src": "assets/novaland/after-1.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/novaland/after-2.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/novaland/after-3.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/novaland/after-4.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/novaland/after-5.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/novaland/after-6.png",
      "alt": "После ремонта",
      "caption": ""
    },
    {
      "src": "assets/novaland/after-7.png",
      "alt": "После ремонта",
      "caption": ""
    }
  ],
  "video": {
    "label": "Видео с комментариями эксперта",
    "embedUrl": null,
    "externalUrl": null,
    "fileUrl": "assets/novaland/expert-overview.mp4",
    "poster": "assets/novaland/after-1.png",
    "note": ""
  },
  "cta": {
    "phone": "+7 (917) 766-09-38",
    "whatsapp": "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
    "telegram": "https://t.me/+79177660938",
    "max": "https://max.ru/chat?phone=%2B79641995283",
    "vk": "https://vk.com/za.komfortom",
    "quiz": "https://zakomfortom.com/quiz",
    "buttonLabel": "Обсудить похожий ремонт"
  }
},
  {
    "id": "trilogiya-120",
    "title": "ЖК Трилогия",
    "address": "Уфа, ул. Аксакова, 2",
    "areaSqm": 120,
    "areaLabel": "120 м²",
    "roomsLabel": "квартира",
    "format": "Дизайн + ремонт под ключ",
    "style": "Современный тёплый интерьер",
    "segment": "Комфорт",
    "complex": "ЖК Трилогия",
    "tags": [
      "120 м²",
      "гостиная",
      "кухня",
      "санузел",
      "видео"
    ],
    "rooms": [
      "гостиная",
      "кухня",
      "спальня",
      "санузел"
    ],
    "hasProjectRender": false,
    "hasVideo": true,
    "cover": "assets/trilogiya-120/after-1.png",
    "summary": "Квартира 120 м² в ЖК Трилогия: дизайн-проект и ремонт под ключ с тёплой нейтральной палитрой, мраморной плиткой, встроенной мебелью и продуманным светом.",
    "task": "Заказчик хотел просторную квартиру с комфортом «как в отеле»: спокойные тёплые оттенки, отдельные зоны для отдыха и приёма гостей, функциональная кухня-столовая, уютные спальни и санузлы без визуального шума. Важно было заранее согласовать отделку, встроенную мебель и освещение по проекту и получить ремонт без сюрпризов на финале.",
    "objectDescription": "На площади 120 м² выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. В гостиной и спальнях — декоративные панели, встроенная мебель и ТВ-зоны, в кухне — фартук из плитки и деревянные фасады, в санузлах — крупноформатная плитка под мрамор. Квартира передана в состоянии, готовом к проживанию.",
    "workList": [
      "Демонтаж старых покрытий и подготовка оснований",
      "Полная замена электрики и сантехники",
      "Штукатурка и выравнивание стен, подготовка под финишную отделку",
      "Устройство стяжки и выравнивание пола",
      "Чистовая отделка: плитка, ламинат/паркет, покраска стен",
      "Монтаж декоративных 3D-панелей и рифлёных стеновых элементов",
      "Монтаж натяжных потолков, трекового и точечного света, люстр и LED-подсветки",
      "Изготовление и монтаж встроенной мебели (кухня, шкафы, ТВ-зоны, гардеробная)",
      "Установка межкомнатных дверей и плинтусов",
      "Монтаж кухонного фартука, сантехники и бытовой техники",
      "Установка розеток, выключателей и финальная уборка"
    ],
    "highlights": [
      "Просторная планировка с тёплой «отельной» палитрой",
      "Мраморная плитка и деревянные акценты в единой стилистике",
      "Встроенная мебель и ТВ-зоны с LED-подсветкой",
      "Кухня-столовая и санузлы в современном формате",
      "Квартира передана готовой к проживанию"
    ],
    "clientFit": [
      "Хочется просторную квартиру, где тепло и спокойно — без «шумной» отделки",
      "Важны мрамор, дерево и встроенная мебель в одной связке, а не разными бригадами",
      "Нужны зоны для гостей и отдыха, где всё выглядит собранно, а не «по частям»",
      "Смотрите на реальный объект, прежде чем выбирать студию"
    ],
    "gallery": [
      {
        "src": "assets/trilogiya-120/after-1.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-2.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-3.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-4.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-5.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-6.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-7.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-8.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-9.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-10.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/trilogiya-120/after-11.png",
        "alt": "После ремонта",
        "caption": ""
      }
    ],
    "video": {
      "label": "Видео с комментариями эксперта",
      "embedUrl": null,
      "externalUrl": null,
      "fileUrl": "assets/trilogiya-120/expert-overview.mp4",
      "poster": "assets/trilogiya-120/after-1.png",
      "note": ""
    },
    "cta": {
      "phone": "+7 (917) 766-09-38",
      "whatsapp": "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
      "telegram": "https://t.me/+79177660938",
      "max": "https://max.ru/chat?phone=%2B79641995283",
      "vk": "https://vk.com/za.komfortom",
      "quiz": "https://zakomfortom.com/quiz",
      "buttonLabel": "Обсудить похожий ремонт"
    }
  },
  {
    "id": "kvartal-entuziastov",
    "title": "ЖК Квартал Энтузиастов",
    "address": "Уфа, ул. Рудольфа Нуреева, 1",
    "areaSqm": null,
    "areaLabel": "площадь уточняется",
    "roomsLabel": "квартира",
    "format": "Дизайн + ремонт под ключ",
    "style": "Современный интерьер с акцентами",
    "segment": "Комфорт",
    "complex": "ЖК Квартал Энтузиастов",
    "tags": [
      "акцентные стены",
      "кухня-гостиная",
      "панорамные окна",
      "видео"
    ],
    "rooms": [
      "кухня-гостиная",
      "спальня",
      "санузел"
    ],
    "hasProjectRender": false,
    "hasVideo": true,
    "cover": "assets/kvartal-entuziastov/after-1.png",
    "summary": "Квартира в ЖК Квартал Энтузиастов: дизайн-проект и ремонт под ключ с выразительными акцентными стенами, встроенной мебелью, трековым светом и панорамными окнами.",
    "task": "Заказчик хотел современную квартиру с характером: не «белая коробка», а продуманные акценты, удобная кухня-столовая, комфортные спальни и рабочая зона, качественная инженерия и понятный результат по проекту. Важно было заранее согласовать отделку, встроенную мебель и освещение и получить ремонт без сюрпризов на финале.",
    "objectDescription": "Выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. В гостиной и спальнях — декоративные панели и 3D-стены с LED-подсветкой, в кухне — двухцветные фасады и стеклянные верхние шкафы, в санузле — плитка под камень и деревянные акценты. Квартира передана в состоянии, готовом к проживанию.",
    "workList": [
      "Демонтаж старых покрытий и подготовка оснований",
      "Полная замена электрики и сантехники",
      "Штукатурка и выравнивание стен, подготовка под финишную отделку",
      "Устройство стяжки и выравнивание пола",
      "Декоративная штукатурка, 3D-панели и рифлёные стеновые элементы",
      "Чистовая отделка: плитка, ламинат/паркет, покраска и обои",
      "Монтаж трекового и точечного света, люстр и LED-подсветки",
      "Изготовление и монтаж встроенной мебели (кухня, шкафы, ТВ-зоны, гардеробная)",
      "Установка скрытых дверей, межкомнатных дверей и плинтусов",
      "Монтаж кухонного фартука, сантехники и бытовой техники",
      "Установка розеток, выключателей и финальная уборка"
    ],
    "highlights": [
      "3D-стены и акцентная отделка с LED-подсветкой",
      "Трековый свет и панорамные окна",
      "Кухня с двухцветными фасадами и стеклянными шкафами",
      "Санузел с плиткой под камень и деревянными акцентами",
      "Ремонт выполнен по дизайн-проекту под ключ"
    ],
    "clientFit": [
      "Не хотите «белую коробку» — нужен интерьер с характером и акцентами",
      "Цените панорамные окна и свет, а не только красивую мебель на рендере",
      "Нужна рабочая зона и комфортные спальни — и один подрядчик от проекта до ключей",
      "Уже сравниваете студии и хотите опереться на сданную квартиру, а не на презентацию"
    ],
    "gallery": [
      {
        "src": "assets/kvartal-entuziastov/after-1.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-2.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-3.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-4.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-5.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-6.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-7.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-8.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-9.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-10.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-11.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-12.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-13.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/kvartal-entuziastov/after-14.png",
        "alt": "После ремонта",
        "caption": ""
      }
    ],
    "video": {
      "label": "Видео с комментариями эксперта",
      "embedUrl": null,
      "externalUrl": null,
      "fileUrl": "assets/kvartal-entuziastov/expert-overview.mp4",
      "poster": "assets/kvartal-entuziastov/after-1.png",
      "note": ""
    },
    "cta": {
      "phone": "+7 (917) 766-09-38",
      "whatsapp": "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
      "telegram": "https://t.me/+79177660938",
      "max": "https://max.ru/chat?phone=%2B79641995283",
      "vk": "https://vk.com/za.komfortom",
      "quiz": "https://zakomfortom.com/quiz",
      "buttonLabel": "Обсудить похожий ремонт"
    }
  },
  {
    "id": "tau-house-2",
    "title": "ЖК Тау Хаус 2",
    "address": "Уфа, ул. Энтузиастов, 7",
    "areaSqm": null,
    "areaLabel": "площадь уточняется",
    "roomsLabel": "квартира",
    "format": "Дизайн + ремонт под ключ",
    "style": "Современная классика",
    "segment": "Комфорт",
    "complex": "ЖК Тау Хаус 2",
    "tags": [
      "семейная",
      "детские",
      "кухня",
      "видео"
    ],
    "rooms": [
      "кухня-гостиная",
      "спальня",
      "санузел"
    ],
    "hasProjectRender": false,
    "hasVideo": true,
    "cover": "assets/tau-house-2/after-1.png",
    "summary": "Квартира в ЖК Тау Хаус 2: дизайн-проект и ремонт под ключ в светлой современной классике — молдинги, рифлёная мебель, многоуровневый свет и продуманные детские комнаты.",
    "task": "Заказчик хотел светлую квартиру для семьи: спокойная палитра, функциональная кухня-столовая и гостиная, уютные спальни и детские с характером, но без перегруза. Важно было заранее согласовать отделку, встроенную мебель и освещение по проекту и получить ремонт под ключ без сюрпризов на финале.",
    "objectDescription": "Выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. В гостиной — ТВ-зона с подсветкой и молдинги, в кухне — рифлёные фасады и мраморный фартук, в санузлах — крупноформатная плитка и встроенная мебель, в детских — авторские фотообои и LED-подсветка. Квартира передана в состоянии, готовом к проживанию.",
    "workList": [
      "Демонтаж старых покрытий и подготовка оснований",
      "Полная замена электрики и сантехники",
      "Штукатурка и выравнивание стен, подготовка под финишную отделку",
      "Устройство стяжки и выравнивание пола",
      "Монтаж стеновых молдингов и декоративных панелей",
      "Чистовая отделка: плитка, ламинат/паркет, покраска стен и фотообои",
      "Монтаж натяжных потолков, трекового и точечного света, LED-подсветки",
      "Изготовление и монтаж встроенной мебели (кухня, шкафы, ТВ-зона)",
      "Установка скрытых и межкомнатных дверей, плинтусов",
      "Монтаж кухонного фартука, сантехники и бытовой техники",
      "Установка розеток, выключателей и финальная уборка"
    ],
    "highlights": [
      "Светлая современная классика с молдингами и рифлёной мебелью",
      "Многоуровневое освещение и ТВ-зона с подсветкой",
      "Детские комнаты с фотообоями и продуманным светом",
      "Санузлы с крупноформатной плиткой и встроенной мебелью",
      "Квартира передана готовой к проживанию"
    ],
    "clientFit": [
      "Семья с детьми — нужны и взрослые зоны, и детские с характером, но без кислотных цветов",
      "Важны молдинги, свет и встроенная мебель — без хаоса из разных бригад",
      "Хотите светлую квартиру, где санузлы и кухня так же продуманы, как гостиная",
      "Ищете подрядчика по живым объектам в новостройках Уфы"
    ],
    "gallery": [
      {
        "src": "assets/tau-house-2/after-1.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-2.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-3.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-4.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-5.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-6.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-7.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-8.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-9.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-10.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-11.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-12.png",
        "alt": "После ремонта",
        "caption": ""
      },
      {
        "src": "assets/tau-house-2/after-13.png",
        "alt": "После ремонта",
        "caption": ""
      }
    ],
    "video": {
      "label": "Видео с комментариями эксперта",
      "embedUrl": null,
      "externalUrl": null,
      "fileUrl": "assets/tau-house-2/expert-overview.mp4",
      "poster": "assets/tau-house-2/after-1.png",
      "note": ""
    },
    "cta": {
      "phone": "+7 (917) 766-09-38",
      "whatsapp": "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
      "telegram": "https://t.me/+79177660938",
      "max": "https://max.ru/chat?phone=%2B79641995283",
      "vk": "https://vk.com/za.komfortom",
      "quiz": "https://zakomfortom.com/quiz",
      "buttonLabel": "Обсудить похожий ремонт"
    }
  },
  {
    id: "green-park-93",
    title: "ЖК Грин Парк",
    address: "Уфа, ул. Менделеева, 156/2",
    areaSqm: 93,
    areaLabel: "93 м²",
    roomsLabel: "3-комнатная квартира",
    format: "Дизайн + ремонт под ключ",
    style: "Современная классика",
    segment: "Комфорт",
    complex: "ЖК Грин Парк",
    tags: [
      "93 м²",
      "гостиная",
      "кухня",
      "спальня",
      "видео"
    ],
    rooms: [
      "гостиная",
      "кухня",
      "спальня",
      "санузел"
    ],
    hasProjectRender: false,
    hasVideo: true,
    cover: "assets/green-park-93/after-1.jpg",
    summary: "Квартира 93 м² в ЖК Грин Парк: дизайн-проект и ремонт под ключ в стиле современной классики — молдинги, декоративная штукатурка, встроенная мебель и многоуровневый свет.",
    task: "Заказчик хотел просторную квартиру с «отельным» уровнем комфорта: спокойная благородная палитра, отдельные зоны для отдыха и приёма гостей, функциональная кухня-столовая, уютная спальня и качественная инженерия. Важно было заранее согласовать отделку, встроенную мебель и освещение по проекту и получить ремонт без сюрпризов на финале.",
    objectDescription: "На площади 93 м² выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. В гостиной и прихожей — декоративная штукатурка и стеновые молдинги с LED-подсветкой, в кухне-столовой — встроенная мебель с рифлёными фасадами и подсветкой, в спальне — акцентные стены и продуманный свет. Квартира передана в состоянии, готовом к проживанию.",
    workList: [
      "Демонтаж старых покрытий и подготовка оснований",
      "Полная замена электрики и сантехники",
      "Штукатурка и выравнивание стен, подготовка под финишную отделку",
      "Устройство стяжки и выравнивание пола",
      "Декоративная штукатурка и монтаж стеновых молдингов",
      "Чистовая отделка: паркет/ламинат, плитка в кухне и санузле, покраска стен",
      "Монтаж натяжных потолков, точечного света, люстр и LED-подсветки",
      "Изготовление и монтаж встроенной мебели (кухня, ТВ-зона, шкафы с подсветкой)",
      "Установка межкомнатных дверей и плинтусов",
      "Монтаж кухонного фартука, сантехники и бытовой техники",
      "Установка розеток, выключателей и финальная уборка"
    ],
    highlights: [
      "Современная классика с молдингами и декоративной штукатуркой",
      "Многоуровневое освещение: люстры, споты и LED-подсветка",
      "Встроенная мебель с рифлёными фасадами и подсветкой",
      "Кухня-столовая и гостиная в единой светлой палитре",
      "Квартира передана готовой к проживанию"
    ],
    clientFit: [
      "Хочется «отельного» уюта — но без китча и лишнего декора",
      "Нужна квартира, где гостям и семье одинаково комфортно",
      "Важны свет, фактуры и встроенная мебель — и один подрядчик от проекта до ключей",
      "Смотрите на реальные объекты, а не только на красивые картинки в Pinterest"
    ],
    gallery: [
      {
        src: "assets/green-park-93/after-1.jpg",
        alt: "После ремонта: Гостиная",
        caption: "Гостиная"
      },
      {
        src: "assets/green-park-93/after-2.jpg",
        alt: "После ремонта: Гостиная, детали",
        caption: "Гостиная, детали"
      },
      {
        src: "assets/green-park-93/after-3.jpg",
        alt: "После ремонта: ТВ-зона",
        caption: "ТВ-зона"
      },
      {
        src: "assets/green-park-93/after-4.jpg",
        alt: "После ремонта: Столовая",
        caption: "Столовая"
      },
      {
        src: "assets/green-park-93/after-5.jpg",
        alt: "После ремонта: Кухня-столовая",
        caption: "Кухня-столовая"
      },
      {
        src: "assets/green-park-93/after-6.jpg",
        alt: "После ремонта: Кухня",
        caption: "Кухня"
      },
      {
        src: "assets/green-park-93/after-7.jpg",
        alt: "После ремонта: Общий вид зон",
        caption: "Общий вид зон"
      },
      {
        src: "assets/green-park-93/after-8.jpg",
        alt: "После ремонта: Спальня",
        caption: "Спальня"
      },
      {
        src: "assets/green-park-93/after-9.jpg",
        alt: "После ремонта: Спальня, рабочая зона",
        caption: "Спальня, рабочая зона"
      }
    ],
    video: {
      label: "Видео с комментариями эксперта",
      embedUrl: null,
      externalUrl: null,
      fileUrl: "assets/green-park-93/expert-overview.mp4",
      poster: "assets/green-park-93/after-1.jpg",
      note: ""
    },
    cta: {
      phone: "+7 (917) 766-09-38",
      whatsapp: "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
      telegram: "https://t.me/+79177660938",
      max: "https://max.ru/chat?phone=%2B79641995283",
      vk: "https://vk.com/za.komfortom",
      quiz: "https://zakomfortom.com/quiz",
      buttonLabel: "Обсудить похожий ремонт"
    }
  },
  {
    id: "nesterovsky-49",
    title: "ЖК Нестеровский",
    address: "Уфа, ул. Гоголя, 29",
    areaSqm: 49,
    areaLabel: "49 м²",
    roomsLabel: "2-комнатная квартира",
    format: "Дизайн + ремонт под ключ",
    style: "Современный светлый интерьер",
    segment: "Комфорт",
    complex: "ЖК Нестеровский",
    tags: [
      "49 м²",
      "кухня",
      "встроенная мебель",
      "санузел",
      "видео"
    ],
    rooms: [
      "кухня",
      "спальня",
      "санузел",
      "коридор"
    ],
    hasProjectRender: false,
    hasVideo: true,
    cover: "assets/nesterovsky-49/after-5.png",
    summary: "Двухкомнатная квартира 49 м² в ЖК Нестеровский: дизайн-проект и ремонт под ключ со светлой отделкой, встроенной мебелью и аккуратной чистовой реализацией.",
    task: "Заказчик хотел обновить двухкомнатную квартиру под современный формат проживания: светлый интерьер, функциональные зоны, качественная инженерная часть и понятный результат по проекту. Важно было заранее согласовать решения по отделке и встроенным элементам и получить ремонт без сюрпризов на финале.",
    objectDescription: "На площади 49 м² выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. Смонтированы потолки с продуманным освещением, встроенная мебель, двери, сантехника. Квартира передана в состоянии, готовом к меблировке.",
    workList: [
      "Демонтаж старых покрытий и подготовка оснований",
      "Полная замена электрики и сантехники",
      "Штукатурка и выравнивание стен, подготовка под финишную отделку",
      "Устройство стяжки и выравнивание пола",
      "Чистовая отделка: напольные покрытия, стены, плитка в санузле",
      "Монтаж натяжных потолков, светильников и LED-подсветки",
      "Изготовление и монтаж встроенной мебели (кухня, шкафы, ТВ-зона)",
      "Установка межкомнатных дверей и плинтусов",
      "Монтаж кухонного фартука и сантехники",
      "Установка розеток, выключателей и финальная уборка"
    ],
    highlights: [
      "Компактная двушка с продуманным хранением",
      "Встроенная мебель и архитектурная LED-подсветка",
      "Светлая отделка и современная кухня",
      "Санузел с душевой и встроенной техникой",
      "Ремонт выполнен по дизайн-проекту под ключ"
    ],
    clientFit: [
      "Живёте в компактной двушке и не хотите терять метры на хаос",
      "Нужен светлый интерьер с умным хранением — без ощущения «ремонт ради ремонта»",
      "Цените, когда инженерия и отделка делаются в одной связке, а не разными бригадами",
      "Уже смотрели подрядчиков и хотите опереться на готовый результат, а не на обещания"
    ],
    gallery: [
      {
        src: "assets/nesterovsky-49/after-1.png",
        alt: "После ремонта: Коридор",
        caption: "Коридор"
      },
      {
        src: "assets/nesterovsky-49/after-2.png",
        alt: "После ремонта: Комната",
        caption: "Комната"
      },
      {
        src: "assets/nesterovsky-49/after-3.png",
        alt: "После ремонта: Спальня со шкафом",
        caption: "Спальня со шкафом"
      },
      {
        src: "assets/nesterovsky-49/after-4.png",
        alt: "После ремонта: Кухня и коридор",
        caption: "Кухня и коридор"
      },
      {
        src: "assets/nesterovsky-49/after-5.png",
        alt: "После ремонта: Кухня",
        caption: "Кухня"
      },
      {
        src: "assets/nesterovsky-49/after-6.png",
        alt: "После ремонта: ТВ-зона",
        caption: "ТВ-зона"
      },
      {
        src: "assets/nesterovsky-49/after-7.png",
        alt: "После ремонта: Санузел, душевая",
        caption: "Санузел, душевая"
      },
      {
        src: "assets/nesterovsky-49/after-8.png",
        alt: "После ремонта: Санузел",
        caption: "Санузел"
      },
      {
        src: "assets/nesterovsky-49/after-9.png",
        alt: "После ремонта: Прихожая",
        caption: "Прихожая"
      },
      {
        src: "assets/nesterovsky-49/after-10.png",
        alt: "После ремонта: Встроенная мебель и отопление",
        caption: "Встроенная мебель и отопление"
      }
    ],
    video: {
      label: "Видео с комментариями эксперта",
      embedUrl: null,
      externalUrl: null,
      fileUrl: "assets/nesterovsky-49/expert-overview.mp4",
      poster: "assets/nesterovsky-49/after-1.png",
      note: ""
    },
    cta: {
      phone: "+7 (917) 766-09-38",
      whatsapp: "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
      telegram: "https://t.me/+79177660938",
      max: "https://max.ru/chat?phone=%2B79641995283",
      vk: "https://vk.com/za.komfortom",
      quiz: "https://zakomfortom.com/quiz",
      buttonLabel: "Обсудить похожий ремонт"
    }
  },
  {
    id: "imperial-3k",
    title: "ЖК Империал",
    address: "Уфа, ул. Аксакова, 16",
    areaSqm: null,
    areaLabel: "3-комнатная · площадь уточняется",
    roomsLabel: "3-комнатная квартира",
    format: "Дизайн + ремонт под ключ",
    style: "Современный светлый интерьер",
    segment: "Комфорт",
    complex: "ЖК Империал",
    tags: [
      "3-комн.",
      "семейная",
      "кухня-гостиная",
      "видео"
    ],
    rooms: [
      "кухня-гостиная",
      "спальня",
      "санузел"
    ],
    hasProjectRender: false,
    hasVideo: true,
    cover: "assets/imperial-3k/after-1.png",
    summary: "Трёхкомнатная квартира в ЖК Империал: дизайн-проект и ремонт под ключ с продуманной планировкой, светлыми материалами и готовностью к проживанию.",
    task: "Заказчик хотел получить современную квартиру для семейного проживания: отдельные зоны для отдыха и работы, функциональная кухня-гостиная, удобная ванная зона, достаточно мест для хранения. Важно было заранее увидеть результат на проекте и получить ремонт без «сюрпризов» по отделке и инженерии.",
    objectDescription: "Выполнены демонтажные работы, новая электрика и сантехника, выравнивание стен и полов, чистовая отделка по дизайн-проекту. Смонтированы потолки, двери, освещение, установлена сантехника и чистовая фурнитура. Квартира сдана в состоянии, готовом к меблировке и заселению.",
    workList: [
      "Демонтаж старых покрытий и подготовка оснований",
      "Полная замена электрики и сантехники",
      "Штукатурка и выравнивание стен, подготовка под покраску и обои",
      "Устройство стяжки и финишное выравнивание пола",
      "Чистовая отделка: напольные покрытия, стены, плитка в мокрых зонах",
      "Монтаж натяжных потолков и светильников",
      "Установка межкомнатных дверей и плинтусов",
      "Монтаж кухонного фартука и сантехники",
      "Установка розеток, выключателей и финальная уборка"
    ],
    plan: {
      src: "assets/imperial-3k/plan.png",
      alt: "Планировка квартиры в ЖК Империал",
      caption: "План квартиры"
    },
    highlights: [
      "Семейная планировка с отдельными зонами",
      "Светлая отделка и качественная инженерная база",
      "Кухня-гостиная и санузлы в современном формате",
      "Ремонт выполнен по дизайн-проекту под ключ",
      "Квартира готова к заселению"
    ],
    clientFit: [
      "Семейная квартира, где каждый метр должен работать на быт, а не на бардак",
      "Нужны отдельные зоны — отдых, работа, гости — без ощущения тесноты",
      "Ремонт планируете один раз и надолго: важны проект, сроки и предсказуемый финал",
      "Выбираете студию по живым объектам, а не по красивой презентации"
    ],
    gallery: [
      {
        src: "assets/imperial-3k/after-1.png",
        alt: "После ремонта: Гостиная",
        caption: "Гостиная"
      },
      {
        src: "assets/imperial-3k/after-2.png",
        alt: "После ремонта: Кухня-гостиная",
        caption: "Кухня-гостиная"
      },
      {
        src: "assets/imperial-3k/after-3.png",
        alt: "После ремонта: Спальня",
        caption: "Спальня"
      },
      {
        src: "assets/imperial-3k/after-4.png",
        alt: "После ремонта: Санузел",
        caption: "Санузел"
      },
      {
        src: "assets/imperial-3k/after-5.png",
        alt: "После ремонта: Прихожая",
        caption: "Прихожая"
      },
      {
        src: "assets/imperial-3k/after-6.png",
        alt: "После ремонта: Детали отделки",
        caption: "Детали отделки"
      },
      {
        src: "assets/imperial-3k/after-7.png",
        alt: "После ремонта: Освещение и потолки",
        caption: "Освещение и потолки"
      },
      {
        src: "assets/imperial-3k/after-8.png",
        alt: "После ремонта: Общий вид",
        caption: "Общий вид"
      }
    ],
    video: {
      label: "Видео с комментариями эксперта",
      embedUrl: null,
      externalUrl: null,
      fileUrl: "assets/imperial-3k/expert-overview.mov",
      poster: "assets/imperial-3k/after-1.png",
      note: ""
    },
    cta: {
      phone: "+7 (917) 766-09-38",
      whatsapp: "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
      telegram: "https://t.me/+79177660938",
      max: "https://max.ru/chat?phone=%2B79641995283",
      vk: "https://vk.com/za.komfortom",
      quiz: "https://zakomfortom.com/quiz",
      buttonLabel: "Обсудить похожий ремонт"
    }
  },
  {
    id: "tau-house-3k",
    title: "ЖК Tau House",
    address: "Уфа, ул. Энтузиастов, 7",
    areaSqm: null,
    areaLabel: "3-комнатная · площадь уточняется",
    roomsLabel: "3-комнатная квартира",
    format: "Дизайн + ремонт под ключ",
    style: "Современный светлый интерьер",
    segment: "Комфорт",
    complex: "ЖК Tau House",
    tags: [
      "3-комн.",
      "светлый интерьер",
      "кухня",
      "видео"
    ],
    rooms: [
      "кухня-гостиная",
      "спальня",
      "санузел"
    ],
    hasProjectRender: false,
    hasVideo: true,
    cover: "assets/tau-house-3k/after-1.png",
    summary: "Квартира в ЖК Tau House: дизайн-проект и ремонт под ключ с аккуратной чистовой отделкой, продуманным освещением и готовностью к заселению.",
    task: "Заказчик хотел современную квартиру «под ключ» без лишней перегрузки в интерьере: светлые тона, функциональные зоны, качественная инженерная часть и понятный результат по проекту. Важно было заранее увидеть планировку решений и получить аккуратную реализацию в срок.",
    objectDescription: "Выполнены демонтажные работы, замена электрики и сантехники, выравнивание оснований и чистовая отделка по дизайн-проекту. Смонтированы потолки, двери, свет, установлена сантехника и чистовая фурнитура. Объект передан в состоянии, готовом к меблировке.",
    workList: [
      "Демонтаж старых покрытий и подготовка оснований",
      "Полная замена электрики и сантехники",
      "Штукатурка и выравнивание стен, подготовка под финишную отделку",
      "Устройство стяжки и выравнивание пола",
      "Чистовая отделка: напольные покрытия, стены, плитка в мокрых зонах",
      "Монтаж натяжных потолков и светильников",
      "Установка межкомнатных дверей и плинтусов",
      "Монтаж кухонного фартука и сантехники",
      "Установка розеток, выключателей и финальная уборка"
    ],
    highlights: [
      "Светлый современный интерьер без перегруза",
      "Аккуратная чистовая отделка и освещение",
      "Функциональные зоны для семейного проживания",
      "Ремонт выполнен по дизайн-проекту под ключ",
      "Квартира готова к меблировке"
    ],
    clientFit: [
      "Любите светлый интерьер без перегруза деталями и «дизайнерского шума»",
      "Хотите снять с себя координацию бригад — один договор, один результат",
      "Важно заранее понимать, как будет выглядеть квартира, а не узнавать на финишной отделке",
      "Ищете подрядчика с реальными сданными квартирами в новостройках Уфы"
    ],
    gallery: [
      {
        src: "assets/tau-house-3k/after-1.png",
        alt: "После ремонта: Гостиная",
        caption: "Гостиная"
      },
      {
        src: "assets/tau-house-3k/after-2.png",
        alt: "После ремонта: Кухня-гостиная",
        caption: "Кухня-гостиная"
      },
      {
        src: "assets/tau-house-3k/after-3.png",
        alt: "После ремонта: Спальня",
        caption: "Спальня"
      },
      {
        src: "assets/tau-house-3k/after-4.png",
        alt: "После ремонта: Санузел",
        caption: "Санузел"
      },
      {
        src: "assets/tau-house-3k/after-5.png",
        alt: "После ремонта: Коридор",
        caption: "Коридор"
      },
      {
        src: "assets/tau-house-3k/after-6.png",
        alt: "После ремонта: Детали отделки",
        caption: "Детали отделки"
      },
      {
        src: "assets/tau-house-3k/after-7.png",
        alt: "После ремонта: Общий вид",
        caption: "Общий вид"
      }
    ],
    video: {
      label: "Видео с комментариями эксперта",
      embedUrl: null,
      externalUrl: null,
      fileUrl: "assets/tau-house-3k/expert-overview.mp4",
      poster: "assets/tau-house-3k/after-1.png",
      note: ""
    },
    cta: {
      phone: "+7 (917) 766-09-38",
      whatsapp: "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
      telegram: "https://t.me/+79177660938",
      max: "https://max.ru/chat?phone=%2B79641995283",
      vk: "https://vk.com/za.komfortom",
      quiz: "https://zakomfortom.com/quiz",
      buttonLabel: "Обсудить похожий ремонт"
    }
  },
  {
    id: "pervomaysky-44",
    title: "ЖК Первомайский",
    address: "Уфа, ул. Первомайская, дом 8",
    areaSqm: 44,
    areaLabel: "44 м²",
    roomsLabel: "1-комнатная квартира",
    format: "Дизайн + ремонт под ключ",
    style: "Современный функциональный интерьер",
    segment: "Комфорт",
    complex: "ЖК Первомайский",
    budgetLabel: "1 429 760 ₽ (на момент реализации)",
    tags: [
      "44 м²",
      "проект / реализация",
      "санузел",
      "кухня-гостиная",
      "малогабаритка",
    ],
    rooms: ["кухня-гостиная", "спальня", "санузел"],
    hasProjectRender: true,
    hasVideo: true,
    cover: "assets/pervomaysky-44/after-8.png",
    summary:
      "Реализованный проект однокомнатной квартиры 44 м² с полным циклом работ: от инженерной базы до чистовой отделки и подключения сантехники.",
    task:
      "У заказчика было пожелание разработать дизайн‑проект с учётом того, что в квартире будет жить студент: все удобства для проживания должны быть продуманы. В санузле — удобный душ и стиральная машина, небольшой кухонный гарнитур и зона отдыха, в спальне — рабочее место и места хранения для личных вещей.",
    objectDescription:
      "Объект площадью 44 м². Квартира подготовлена к проживанию: выполнены инженерные и отделочные работы, установлены чистовые элементы и сантехническое оборудование.",
    workList: [
      "Полная замена электрики и сантехники",
      "Штукатурка стен и подготовка под покраску",
      "Финишное выравнивание пола",
      "Плиточные работы в санузле",
      "Монтаж натяжных потолков и установка светильников",
      "Монтаж и покраска декоративных элементов на стенах",
      "Установка чистовых розеток и выключателей",
      "Установка и подключение сантехники",
    ],
    plan: {
      src: "assets/pervomaysky-44/plan.png",
      alt: "Планировка квартиры в ЖК Первомайский",
      caption: "План квартиры с экспликацией помещений",
    },
    highlights: [
      "Проект и реализация выполнены в единой стилистике",
      "Сделан акцент на инженерной надежности и аккуратной чистовой отделке",
      "Санузел и кухня-гостиная собраны в современном функциональном формате",
      "Квартира передана полностью готовой к проживанию",
    ],
    clientFit: [
      "Студент или молодой специалист — нужна квартира «под жизнь», а не под каталог мебели",
      "Важны рабочее место, душ, стиралка и кухня — всё на небольшой площади",
      "Не хочется самому собирать пазл из отделки, электрики и сантехники",
      "Хотите увидеть, как решена похожая планировка, прежде чем подписывать смету"
    ],
    gallery: [
      {
        src: "assets/pervomaysky-44/after-1.png",
        alt: "После ремонта: спальня и рабочая зона",
        caption: "Спальня и рабочая зона",
      },
      {
        src: "assets/pervomaysky-44/after-2.png",
        alt: "После ремонта: гардеробная/прихожая зона",
        caption: "Входная зона",
      },
      {
        src: "assets/pervomaysky-44/after-3.png",
        alt: "После ремонта: зона гостиной",
        caption: "Гостиная",
      },
      {
        src: "assets/pervomaysky-44/after-4.png",
        alt: "После ремонта: кухня-гостиная",
        caption: "Кухня-гостиная",
      },
      {
        src: "assets/pervomaysky-44/after-5.png",
        alt: "После ремонта: обеденная группа",
        caption: "Обеденная зона",
      },
      {
        src: "assets/pervomaysky-44/after-6.png",
        alt: "После ремонта: санузел",
        caption: "Санузел",
      },
      {
        src: "assets/pervomaysky-44/after-7.png",
        alt: "После ремонта: общий вид кухни-гостиной",
        caption: "Общий вид кухни-гостиной",
      },
      {
        src: "assets/pervomaysky-44/after-8.png",
        alt: "После ремонта: гардеробная зона",
        caption: "Гардеробная зона",
      },
      {
        src: "assets/pervomaysky-44/after-9.png",
        alt: "После ремонта: зона у окна",
        caption: "Зона у окна",
      },
      {
        src: "assets/pervomaysky-44/after-10.png",
        alt: "После ремонта: кухня",
        caption: "Кухня",
      },
      {
        src: "assets/pervomaysky-44/after-11.png",
        alt: "После ремонта: душевая",
        caption: "Душевая зона",
      },
      {
        src: "assets/pervomaysky-44/after-12.png",
        alt: "После ремонта: санузел с раковиной",
        caption: "Санузел с тумбой и раковиной",
      },
    ],
    beforeGallery: [
      {
        src: "assets/pervomaysky-44/before-1.png",
        alt: "Квартира до ремонта",
        caption: "До ремонта: помещение",
      },
      {
        src: "assets/pervomaysky-44/before-2.png",
        alt: "Квартира до ремонта: инженерная часть",
        caption: "До ремонта: сантехнические работы",
      },
      {
        src: "assets/pervomaysky-44/before-3.png",
        alt: "Квартира до ремонта: черновой этап",
        caption: "До ремонта: черновой этап",
      },
    ],
    compareGallery: [
      {
        src: "assets/pervomaysky-44/compare-1.png",
        alt: "Проект и реализация: общий коллаж",
        caption: "Проект / реализация",
      },
      {
        src: "assets/pervomaysky-44/compare-2.png",
        alt: "Проект и реализация: спальня",
        caption: "Спальня: проект / реализация",
      },
      {
        src: "assets/pervomaysky-44/compare-3.png",
        alt: "Проект и реализация: кухня-гостиная",
        caption: "Кухня-гостиная: проект / реализация",
      },
      {
        src: "assets/pervomaysky-44/compare-4.png",
        alt: "Проект и реализация: рабочая зона",
        caption: "Рабочая зона: проект / реализация",
      },
      {
        src: "assets/pervomaysky-44/compare-5.png",
        alt: "Проект и реализация: санузел",
        caption: "Санузел: проект / реализация",
      },
    ],
    video: {
      label: "Видео с комментариями эксперта",
      embedUrl: null,
      externalUrl: null,
      fileUrl: "assets/pervomaysky-44/expert-overview.mp4",
      poster: "assets/pervomaysky-44/after-1.png",
      note: "",
    },
    cta: {
      phone: "+7 (917) 766-09-38",
      whatsapp:
        "https://wa.me/79177660938?text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5%21%20%D0%9D%D0%B0%D1%88%D0%B5%D0%BB%20%D0%B2%D0%B0%D1%88%20%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%20%D0%BF%D0%B5%D1%80%D0%B5%D0%B9%D0%B4%D1%8F%20%D1%87%D0%B5%D1%80%D0%B5%D0%B7%20%D1%81%D0%B0%D0%B9%D1%82",
      telegram: "https://t.me/+79177660938",
      max: "https://max.ru/chat?phone=%2B79641995283",
      vk: "https://vk.com/za.komfortom",
      quiz: "https://zakomfortom.com/quiz",
      buttonLabel: "Обсудить похожий ремонт",
    },
  },
];

/** Уникальные ЖК из базы */
function getComplexes() {
  const set = new Set(CASES.map((c) => c.complex));
  return Array.from(set).sort();
}
