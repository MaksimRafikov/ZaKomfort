...
Статический каталог кейсов ремонта: HTML/CSS/JS. Источник данных — `content/cases/*.json`, `content/site.json` и `js/tips-data.js`; `js/data.js`, `js/config.js`, `cases/<slug>/index.html`, `tips/<slug>/index.html` и `sitemap.xml` генерируются сборкой.
...
4. `python scripts/build-pages.py` генерирует `js/data.js` (с полями `coverSrcset`/`coverWebpSrcset` для карточек), статические страницы `cases/<slug>/index.html` с `<picture>`+srcset, страницы советов `tips/<slug>/index.html`, `sitemap.xml` (lastmod — дата последнего коммита файла) и проставляет кэш-версии `?v=<hash>` во всех HTML — вручную версии не редактируются.
